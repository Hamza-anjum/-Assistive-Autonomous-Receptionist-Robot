...
/*****ultrasonicS***/
#define echoPin A13 // attach pin A13 Arduino to pin Echo of HC-SR04
#define trigPin A14 //attach pin A14 Arduino to pin Trig of HC-SR04
long duration; // variable for the duration of sound wave travel
int distance; // variable for the distance measurement

#define LechoPin A11 // attach pin A11 Arduino to pin Echo of HC-SR04
#define LtrigPin A12 //attach pin A12 Arduino to pin Trig of HC-SR04
long Lduration; // variable for the duration of sound wave travel
int Ldistance; // variable for the distance measurement

#define RechoPin A9 // attach pin A9 Arduino to pin Echo of HC-SR04
#define RtrigPin A10 //attach pin A10 Arduino to pin Trig of HC-SR04
long Rduration; // variable for the duration of sound wave travel
int Rdistance; // variable for the distance measurement
 ...
 */
#define FWD       0xFF18E7 // go forward(2)
#define LFT       0xFF10EF // go left(4)
#define RGT       0xFF5AA5 // go right(6)
#define BWD       0xFF4AB5 // go backward(8)
#define STOP      0xFF629D // stop
#define RPEAT     0xFF02FD // repeat the stored sequence of movement from the temporary memory(automatically stores) (back)
#define DEL       0xFFE21D // delete the stored sequence of movement from temporary memory(mute)
#define PERST     0xFFE01F // copy the sequence from temp. memory to the peramanent memory(prev)
#define PLAYEPROM 0xFFA857 // repeat the sequence stored in EEPROM(next)
#define RESET     0xFF22DD // Resets the Arduino Board(mode)
#define RUN       0xFF906F // play stored path in while loop (play/pause)          
...
byte seq_Array[500];// array to store the movement sequence in terms of integers(1 for FWD, 2 for LEFT and so on..)
...
unsigned long int total_Bwd_Time[50];
unsigned long int total_Stp_Time[50];
...
//reading permanent memory in loop


void Read_Permt_loop(){
  // Read from permanent memory
   byte x = EEPROM.read(100);
   for (int h=0; h<20; h++)
    {
...
void movement_Inst_Fwd(void) {
  obstacle_check();
  while (distance < 40)
    {Serial.print("waiting for obstacle to move");
     delay(3000);
     obstacle_check();
     }
  ...
  // Spin the stepper :
  for (int i = 0; i < 2.5 * stepsPerRevolution; i++) {
  ...
  LEFT_obstacle_check();
  while (Ldistance < 20)
    {Serial.print("moving bit right");
     movement_fwd_Rgt();
     delay(1000);
     LEFT_obstacle_check(); }
  RIGHT_obstacle_check();   
  while (Rdistance < 20)
    {Serial.print("moving bit left");
     movement_fwd_Lft();
     delay(1000);
     RIGHT_obstacle_check(); }      
}
...
void RIGHT_obstacle_check (void) {  
  // Clears the trigPin condition
